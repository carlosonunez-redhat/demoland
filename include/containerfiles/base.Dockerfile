...
ENV OPENSHIFT_BINARIES_URL="https://mirror.openshift.com/pub/openshift-v4/multi/clients/ocp/${OPENSHIFT_VERSION}"
RUN \
  arch=amd64; \
  uname -m | grep -Eq 'arm|aarch' && arch=arm64; \
  url="${OPENSHIFT_BINARIES_URL}/$arch/openshift-client-linux"; \
  curl -v -o /tmp/file.tar.gz "${url}.tar.gz"
...
RUN dnf -y install yq jq openssh ssh-agent htpasswd bsdtar docker podman
