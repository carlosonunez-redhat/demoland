locals {
  user_data_bootstrap = jsonencode({
    ignition = {
      version = "3.1.0"
      config = {
        replace = {
          source = "http://${local.bootstrap_bucket_name}.s3.${data.aws_region.current.region}.amazonaws.com/openshift_install/bootstrap.ign"
        }
      }
    }
  })
  user_data_control_plane = jsonencode({
    ignition = {
      version = "3.1.0"
      config = {
        merge = [{
          source = "http://${local.bootstrap_bucket_name}.s3.${data.aws_region.current.region}.amazonaws.com/openshift_install/master.ign"
        }]
        security = {
          tls = {
            certificateAuthorities = jsondecode(var.control_plane_cert_bundle)
          }
        }
      }
    }
  })
  user_data_worker = jsonencode({
    ignition = {
      version = "3.1.0"
      config = {
        merge = [{
          source = "http://${local.bootstrap_bucket_name}.s3.${data.aws_region.current.region}.amazonaws.com/openshift_install/worker.ign"
        }]
        security = {
          tls = {
            certificateAuthorities = jsondecode(var.worker_cert_bundle)
          }
        }
      }
    }
...
module "disconnected-ocp-bootstrap-node" {
  ...
  instance_type = local.options.cloud_config.aws.compute.instance_sizes.vm
  ami = jsondecode(data.http.rhcos.response_body).architectures.x86_64.images.aws.regions[data.aws_region.current.region].image
  key_name = module.ec2_key.key_pair_name
  subnet_id = local.provisioning_subnet_disconnected
  vpc_security_group_ids = [
...
module "disconnected-ocp-cp-nodes-bm" {
  ...
  user_data = local.user_data_control_plane
  timeouts = {
    create = "15m"
    update = "15m"
    delete = "15m"
